...
- 工作区恢复只验证顶层根目录；展开目录时才由 Go 按层读取子项，避免大型文档库启动时全量扫描。
...
- 在非输入框区域按 `Ctrl/Cmd + A` 只选择 Markdown 正文，不包含文档标题、路径和工具栏。
...
- 正文搜索结果会显示命中行号，打开结果后自动滚动并高亮对应命中。
...
- 自动同步只刷新用户已经展开加载的目录层级，普通正文修改和未展开深层目录不会产生周期性扫描开销。
- 新发现的子目录默认保持收起。
...
- 工具栏提供统一设置入口，语言和主题不再分散占用工具栏空间。
- 设置弹窗支持浅色、深色、跟随系统主题，以及中文和英文界面；语言文案和偏好保存仍由 Go 统一管理。
- Windows 设置中可查看 Markdown 文件关联状态，并打开系统默认应用页面进行修复或选择。
...
- 安装器会根据系统语言显示中文或英文界面。
- 升级安装时优先复用已安装目录。
- 安装器会使用已有管理员权限注册 Windows Explorer 右键菜单，并把 JS Kern.md 注册为 `.md`、`.markdown`、`.mdown` 的可选默认应用。
- Windows 要求用户在系统设置中确认实际默认应用；安装器不会绕过系统保护强制修改 `UserChoice`。
- 卸载器只清理 JS Kern.md 自己创建的注册表键和值。
...
- 修复点击“修复或设置默认应用”后错误打开文件资源管理器的问题。
- Windows 设置 URI 改由系统原生 ShellExecute 打开，不再把 URI 交给 `explorer.exe` 当作文件路径处理。
- 精确应用设置页无法启动时自动回退到 Windows 默认应用主页。
